...
## Hard rule — never ship a diagram with placeholders

Every Diagram template carries **many** editable text layers beyond `diagram-title` and `diagram-body` — node labels like `Title`, `This is a paragraph`, `Bullet Point 1/2/3`, `small note`, `Server`, plus process-step and swimlane labels. Cloning a Diagram and only setting the title/body ships placeholder copy on every node and is rejected on review.

Before adding any Diagram slide to a deck plan:

1. **Confirm the brief contains real content for every visible node** — not just a title and one-line description.
2. If the data isn't there, **drop the slide** or substitute a text-based variant (`About Nagarro - Headline + 2 Col Body / Light`, `Content - Title + Body`, etc.).
3. Treat dense variants (Architecture / Complex Flow, Process Roadmap, Support Flow) as **clone-and-manual-edit only** — they should never appear in an auto-build script.

This rule applies to both Figma and PowerPoint builds.
